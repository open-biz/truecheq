...
export function AppShell({ initialTab = 'sell' }: AppShellProps) {
  ...
  useEffect(() => {
    ...
    // No stored user — check MiniKit for wallet info without triggering SIWE
    ...
    if (walletAddress) {
      // Seed a guest user with the wallet info already available from MiniKit
      const guestUser: TruCheqUser = {
        nullifierHash: 'guest',
        isOrbVerified,
      ...
      setUser(guestUser);
      saveTruCheqUser(guestUser);
      // Persist the address so other components can read it
      try {
